"""
step_walkforward_monthly_weighted_v4_brent_divret_cashonly.py

Идентично v4_brent_divret, но в дни go_cash деньги лежат
на брокерском счету (ret=0.0) — без перекладывания на ставку ЦБ.

Позволяет оценить чистый вклад go_cash-тайминга без эффекта
безрисковой ставки.

Вход:  dataset_vol_brent.parquet + dividends_moex.csv
Выход: equity_curve_wf_monthly_v4_brent_divret_cashonly.csv
       backtest_report_wf_monthly_v4_brent_divret_cashonly.txt
       wf_monthly_v4_brent_divret_models/  (те же модели)
"""
...
EQUITY_PATH   = os.path.join(BASE_DIR, "equity_curve_wf_monthly_v4_brent_divret_cashonly.csv")
REPORT_PATH   = os.path.join(BASE_DIR, "backtest_report_wf_monthly_v4_brent_divret_cashonly.txt")
MODELS_DIR    = os.path.join(BASE_DIR, "wf_monthly_v4_brent_divret_models")
...
print("=" * 60)
print("WF Monthly v4_brent_divret + cashonly (go_cash → ret=0)")
print("=" * 60)
...
FILTER_WINDOW_DAYS   = 252
SENT_PERCENTILE_SOFT = 20
SENT_PERCENTILE_HARD = 10

CBR_RATE_THRESHOLD_1 = 18.0
CBR_RATE_THRESHOLD_2 = 20.0
...
# Строим словарь: date → {ticker: dividend_yield}
# dividend_yield = value / close на дату отсечки
price_on_date = df.set_index(["ticker", "date"])["close"]

div_map = {}  # date → {ticker: div_yield}
for _, row in divs_raw.iterrows():
    ...
    try:
        ...
        # Дата отсечки не является торговым днём — берём ближайший торговый день
...
print(f"Дат с дивидендами в бэктесте: {len(div_map)}")

print(f"Данных: {len(df):,} строк, тикеров: {df['ticker'].nunique()}")
print(f"Период: {df['date'].min().date()} → {df['date'].max().date()}")
print(f"Признаков: {len(FEATURE_COLS)} (35 базовых + 3 Brent)")
print(f"Адаптивный фильтр: окно {FILTER_WINDOW_DAYS} дней")
print(f"CBR-фильтр: >={CBR_RATE_THRESHOLD_1}% → x0.5, >={CBR_RATE_THRESHOLD_2}% → x0.25")
...
for fold_idx, test_start in enumerate(test_months):
    ...
    preds = test[["date", "ticker", "lgb_pred", "target",
                  "market_sent_7d", "imoex_ret_20d"]].copy()
...
def cbr_pos_scale(rate):
    if rate > CBR_RATE_THRESHOLD_2:
        return 0.25
    elif rate > CBR_RATE_THRESHOLD_1:
        return 0.5
    return 1.0
...
sent_window  = deque()
sent_history = {}
daily_sent   = pred_df.groupby("date")["market_sent_7d"].mean().sort_index()
for date in dates:
    sent_history[date] = daily_sent.get(date, 0.0)
...
for date in dates:
    ...
    market_sent  = sent_history[date]
    imoex_ret    = day["imoex_ret_20d"].mean()
    cbr_daily    = cbr_rates.get(date, 0.0)
    cur_cbr_rate = cbr_rate_map.get(date, cbr_rate_map.iloc[-1])

    sent_window.append(market_sent)
    if len(sent_window) > FILTER_WINDOW_DAYS:
        sent_window.popleft()

    window_arr = np.array(sent_window)
    sent_hard  = np.percentile(window_arr, SENT_PERCENTILE_HARD)
    sent_soft  = np.percentile(window_arr, SENT_PERCENTILE_SOFT)

    go_cash = (market_sent < sent_hard) or \
              (market_sent < sent_soft and imoex_ret < 0)
    ...
    pos_scale  = cbr_pos_scale(cur_cbr_rate)
    cash_share = 1.0 - pos_scale
    ...
    # Дивидендный доход: если сегодня дата отсечки у тикера в портфеле
    todays_divs = div_map.get(date, {})
    div_ret_w = sum(cur_w.get(t, 0.0) * dy for t, dy in todays_divs.items())
    port_ret += div_ret_w

    returns_w.append({"date": date,
                       "ret": port_ret * pos_scale + cbr_daily * cash_share,
                       "in_market": True,
                       "div_ret": div_ret_w})
    prev_w = cur_w
    ...
    eq_ret += div_ret_eq
    returns_eq.append({"date": date,
                        "ret": eq_ret * pos_scale + cbr_daily * cash_share,
                        "in_market": True})
    prev_eq = cur_eq
...
m_w   = calc_metrics(df_w["ret"],  "v4_brent_divret средневзв.")
m_eq  = calc_metrics(df_eq["ret"], "v4_brent_divret равновзв.")
...
print(f"\n{'─'*80}")
print(f"{'Метрика':<25} {'divret средневзв.':>18} {'divret равновзв.':>16} {'ЦБ':>8}" +
...
cash_days = df_w[~df_w["in_market"]].shape[0]
print(f"\nДней в кэше: {cash_days} ({100*cash_days/len(df_w):.1f}%)")
...
print(f"\n{'─'*50}")
print(f"\nДля сравнения:")
print(f"  v4_brent_divret (со ставкой ЦБ): Sharpe ~2.31 | CAGR ~+55%")
print(f"  cashonly (без ставки ЦБ):        текущий результат выше")
print(f"{'─'*50}")
...
with open(REPORT_PATH, "w", encoding="utf-8") as f:
    f.write("Walk-Forward Backtest Report (monthly, v4_divs + Brent, cashonly)\n")
    f.write("go_cash → деньги на брокерском счету (ret=0), не на ставке ЦБ\n")
    f.write("=" * 60 + "\n")
    f.write("Добавлены признаки нефти Brent (Yahoo Finance, BZ=F):\n")
    f.write("  brent_ret_5d   — 5d return\n")
    f.write("  brent_ret_20d  — 20d return\n")
    f.write("  brent_vol_20d  — 20d volatility annualized\n\n")
    f.write(f"Признаков: {len(FEATURE_COLS)} (35 базовых + 3 Brent)\n")
    f.write(f"COMMISSION={COMMISSION}, SOFTMAX_TEMP={SOFTMAX_TEMP}\n\n")

"""
step_walkforward_monthly_weighted_vol.py

WF monthly — взвешенный по уверенности (temp=0.1) + объёмные фичи.

Вход:  dataset_vol.parquet  (создаётся step2b_add_volume.py)
Выход: equity_curve_wf_monthly_weighted_vol.csv
       backtest_report_wf_monthly_weighted_vol.txt
       wf_monthly_vol_models/
"""
...
EQUITY_PATH   = os.path.join(BASE_DIR, "equity_curve_wf_monthly_weighted_vol.csv")
REPORT_PATH   = os.path.join(BASE_DIR, "backtest_report_wf_monthly_weighted_vol.txt")
MODELS_DIR    = os.path.join(BASE_DIR, "wf_monthly_vol_models")
...
print("=" * 60)
print("WF Monthly — взвешенный + объёмные фичи")
print("=" * 60)
...
if not os.path.exists(INPUT_PARQUET):
    raise FileNotFoundError(
        f"Не найден {INPUT_PARQUET}\n"
        f"Сначала запустите: python step2b_add_volume.py"
    )
...
vol_coverage = (df["volume_ratio_5d"] != 0).mean()
print(f"Покрытие объёмом: {vol_coverage:.1%}")
...
for fold_idx, test_start in enumerate(test_months):
    ...
    if len(train_fit) < 1000:
        print(f"  Фолд {fold_idx+1:2d}: мало данных, пропуск")
        continue
    if len(test) == 0:
        continue
...
returns_w  = []   # взвешенный с объёмом
returns_eq = []   # равновзвешенный baseline
prev_w, prev_eq = {}, set()

for date in dates:
    if len(sent_history) >= MIN_HIST_DAYS:
        market_threshold = np.percentile(sent_history, MARKET_PERCENTILE)
    else:
        market_threshold = -np.inf
    ...
    if market_sent < market_threshold:
        returns_w.append( {"date": date, "ret": cbr_daily, "in_market": False})
        returns_eq.append({"date": date, "ret": cbr_daily, "in_market": False})
        continue

    top = day.sort_values("lgb_pred", ascending=False).head(TOP_N)

    if len(top) == 0:
        returns_w.append( {"date": date, "ret": 0.0, "in_market": False})
    ...
    all_tickers = set(cur_w) | set(prev_w)
    turnover = sum(abs(cur_w.get(t, 0.0) - prev_w.get(t, 0.0)) for t in all_tickers) / 2
    returns_w.append({"date": date, "ret": (top["target"].values * weights).sum() - turnover * COMMISSION, "in_market": True})
    prev_w = cur_w

    # Равновзвешенный
    cur_eq = set(top["ticker"])
    to_eq  = len(cur_eq.symmetric_difference(prev_eq)) / max(len(cur_eq), 1)
    returns_eq.append({"date": date, "ret": top["target"].mean() - to_eq * COMMISSION, "in_market": True})
    prev_eq = cur_eq
...
m_w   = calc_metrics(df_w["ret"],  f"Weighted+Vol (TOP{TOP_N}, temp={SOFTMAX_TEMP})")
m_eq  = calc_metrics(df_eq["ret"], f"Equal weight (TOP{TOP_N})")
...
print(f"\n{'─'*80}")
print(f"{'Метрика':<25} {'Weighted+Vol':>16} {'Equal weight':>14} {'ЦБ':>8}" +
...
with open(REPORT_PATH, "w", encoding="utf-8") as f:
    f.write("Walk-Forward Backtest Report (monthly, weighted + volume)\n")
    f.write("=" * 60 + "\n")
    f.write(f"TOP_N={TOP_N}, p{MARKET_PERCENTILE}, COMMISSION={COMMISSION}, SOFTMAX_TEMP={SOFTMAX_TEMP}\n")
    f.write(f"Фичи: базовые + volume_ratio_5d + volume_ratio_20d\n\n")

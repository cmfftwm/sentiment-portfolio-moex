"""
step_walkforward_monthly_nosent_v4_brent_divret_nogocash.py

БЕЗ сентимента, БЕЗ ухода в кэш (ни go_cash, ни CBR pos_scale).
Цель: базовая линия для изоляции вклада сентимента в выбор акций.

Вход:  dataset_vol_brent.parquet + dividends_moex.csv
Выход: equity_curve_wf_monthly_nosent_v4_brent_divret_nogocash.csv
       backtest_report_wf_monthly_nosent_v4_brent_divret_nogocash.txt
       wf_monthly_nosent_v4_brent_divret_nogocash_models/
"""
...
EQUITY_PATH   = os.path.join(BASE_DIR, "equity_curve_wf_monthly_nosent_v4_brent_divret_nogocash.csv")
REPORT_PATH   = os.path.join(BASE_DIR, "backtest_report_wf_monthly_nosent_v4_brent_divret_nogocash.txt")
MODELS_DIR    = os.path.join(BASE_DIR, "wf_monthly_nosent_v4_brent_divret_nogocash_models")
...
print("=" * 60)
print("WF Monthly БЕЗ сентимента БЕЗ go_cash")
print("=" * 60)
...
# Только технические + макро + дивиденды + Brent — БЕЗ сентимента
FEATURE_COLS = [
...
print(f"Признаков: {len(FEATURE_COLS)} (БЕЗ сентимента)")
...
m_w   = calc_metrics(df_w["ret"],  "nosent_nogocash средневзв.")
m_eq  = calc_metrics(df_eq["ret"], "nosent_nogocash равновзв.")
...
print(f"\n{'─'*80}")
print(f"{'Метрика':<25} {'nosent средневзв.':>18} {'nosent равновзв.':>16} {'ЦБ':>8}" +
...
print(f"\n{'─'*50}")
print(f"\nДля сравнения (с сентиментом, без go_cash):")
print(f"  sent_nogocash: запустить step_walkforward_monthly_weighted_v4_brent_divret_nogocash.py")
print(f"{'─'*50}")
...
with open(REPORT_PATH, "w", encoding="utf-8") as f:
    f.write("Walk-Forward Backtest Report (БЕЗ сентимента БЕЗ go_cash)\n")
    f.write("=" * 60 + "\n")
    f.write("БЕЗ СЕНТИМЕНТА. go_cash ОТКЛЮЧЁН, CBR pos_scale ОТКЛЮЧЁН.\n")
    f.write(f"Признаков: {len(FEATURE_COLS)} (технич. + макро + дивиденды + Brent)\n\n")

...
Walk-forward validation: переобучаем LightGBM каждую НЕДЕЛЮ,
предсказываем только следующую неделю (честный OOS бэктест).

Вход:  dataset.parquet
Выход: equity_curve_wf_weekly.csv + backtest_report_wf_weekly.txt + wf_weekly_models/
"""
...
EQUITY_PATH   = os.path.join(BASE_DIR, "equity_curve_wf_weekly.csv")
REPORT_PATH   = os.path.join(BASE_DIR, "backtest_report_wf_weekly.txt")
MODELS_DIR    = os.path.join(BASE_DIR, "wf_weekly_models")
...
print("=" * 60)
print("Walk-Forward Validation (еженедельное переобучение)")
print("=" * 60)
...
test_months = pd.date_range(start=TEST_START, end=df["date"].max(), freq="W-MON")

print(f"\nФолдов (недель): {len(test_months)}")
...
for fold_idx, test_start in enumerate(test_months):
    test_end  = test_start + pd.offsets.Week()
    train_end = test_start - pd.Timedelta(days=1)

    train = df[df["date"] <= train_end].copy()
    test  = df[(df["date"] >= test_start) & (df["date"] <= test_end)].copy()

    # Исключаем последние 5 торговых дней: их target_5d захватывает тестовую неделю
    ...
    print(f"  Фолд {fold_idx+1:3d} ({test_start.strftime('%Y-%m-%d')}): "
...
m_wf = calc_metrics(df_wf["ret"], f"WF weekly LightGBM-5d (TOP{TOP_N}, p{MARKET_PERCENTILE})")
...
print(f"\n{'─' * 80}")
print(f"{'Метрика':<25} {'WF weekly LGB':>18} {'v1 сентимент':>14} {'ЦБ':>8}" +

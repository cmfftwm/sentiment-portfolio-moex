"""
step_walkforward_monthly_weighted_v4_brent_divret.py

v4_brent + учёт получения дивидендов в бэктесте.

В дневном цикле: если тикер в портфеле имеет дату отсечки сегодня —
добавляем dividend/price к доходу позиции. ML-модель и признаки не меняются.

Вход:  dataset_vol_brent.parquet + dividends_moex.csv
Выход: equity_curve_wf_monthly_v4_brent_divret.csv
       backtest_report_wf_monthly_v4_brent_divret.txt
       wf_monthly_v4_brent_divret_models/
"""
...
EQUITY_PATH   = os.path.join(BASE_DIR, "equity_curve_wf_monthly_v4_brent_divret.csv")
REPORT_PATH   = os.path.join(BASE_DIR, "backtest_report_wf_monthly_v4_brent_divret.txt")
...
print("=" * 60)
print("WF Monthly v4_brent + учёт получения дивидендов")
print("=" * 60)
...
for date in dates:
    ...
    if go_cash:
        returns_w.append( {"date": date, "ret": cbr_daily, "in_market": False, "div_ret": 0.0})
        returns_eq.append({"date": date, "ret": cbr_daily, "in_market": False})
        continue
...
print(f"\n{'─'*50}")
print(f"\nДля сравнения:")
print(f"  v4_brent (без дивидендов): Sharpe 2.166 | CAGR +50.3% | Max DD -25.9%")
print(f"{'─'*50}")
...
with open(REPORT_PATH, "w", encoding="utf-8") as f:
    f.write("Walk-Forward Backtest Report (monthly, v4_divs + Brent)\n")
    f.write("=" * 60 + "\n")
